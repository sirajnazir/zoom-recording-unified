...
class DriveRecordingProcessor {
  ...
  async checkDuplicate(session) {
    if (!this.config.google.sheets.masterIndexSheetId) {
      return false;
    }
    ...
    try {
      ...
      try {
        const spreadsheet = await this.sheets.spreadsheets.get({
          spreadsheetId: this.config.google.sheets.masterIndexSheetId
        });
      ...
      for (const row of rows.slice(1)) {
        ...
        if (session.metadata.date && existingDate === session.metadata.date.raw) {
  ...
  async processSession(session) {
    ...
    const standardizedName = await this.generateStandardizedName(session);
    ...
    if (session.metadata.hasTranscript) {
  ...
  generateFingerprint(session) {
    ...
    if (session.metadata.date && session.metadata.date.raw) {
  ...
  async generateStandardizedName(session) {
    ...
    if (session.metadata.week && session.metadata.week.number) {
      parts.push(`Week${session.metadata.week.number}`);
    }
    
    parts.push(`UUID-${session.metadata.fingerprint.substring(0, 8)}`);
  ...
  async determineTargetFolder(session) {
    const rootFolderId = this.config.google.drive.recordingsRootFolderId;
    ...
    if (participants.length >= 2) {
      ...
      if (this.isCoach(person1) || this.isCoach(person2)) {
        categoryFolder = this.config.google.drive.coachesFolderId;
      } else {
        categoryFolder = this.config.google.drive.studentsFolderId;
      }
    } else if (session.metadata.confidence < 40) {
      categoryFolder = this.config.google.drive.trivialFolderId;
    } else {
      categoryFolder = this.config.google.drive.miscFolderId;
    }
  ...
  async updateGoogleSheet(processedSession) {
    ...
    try {
      const values = [[
        ...
        processedSession.processing.source,
...
module.exports = DriveRecordingProcessor;