...
class GoogleSheetsService {
    constructor(config, dependencies = {}) {
        ...
        this.logger = dependencies.logger || new Logger('GoogleSheetsService');
        this.cache = dependencies.cache || new Cache();
        this.metrics = dependencies.metricsCollector || new MetricsCollector();
        
        // Enhanced services for smart data
        this.nameStandardizer = dependencies.nameStandardizer;
        this.weekInferencer = dependencies.weekInferencer;
        this.metadataExtractor = dependencies.metadataExtractor;
        this.transcriptionAnalyzer = dependencies.transcriptionAnalyzer;
        ...
        // Spreadsheet configuration - use correct config structure
        this.spreadsheetId = config.sheets?.masterIndexSheetId;
        this.sheetName = config.sheets?.sheetName || 'Smart Schema Master Index';
        
        // Column mappings for smart schema
        this.columns = this._defineColumns();
        
        // Processing configuration
        this.batchSize = config.sheets?.batchSize || 100;
        this.rateLimitDelay = config.sheets?.rateLimitDelay || 100;
    ...
    async _initialize() {
        try {
            ...
            throw new GoogleSheetsError('Initialization failed', error);
        }
    }
    ...
    async _initializeSpreadsheet() {
        try {
            ...
            throw new GoogleSheetsError('Cannot initialize spreadsheet', error);
        }
    }
    ...
    async updateMasterSpreadsheet(processedRecording, source = 'Reprocessing') {
        ...
        try {
            ...
            if (await this._isDuplicate(smartData.uuid, smartData.fingerprint)) {
                ...
                this.metrics.increment('sheets.duplicates.skipped');
                return {
                    success: true,
                    duplicate: true,
            ...
            this.metrics.increment('sheets.updates.successful');
            ...
            this.metrics.increment('sheets.updates.failed');
