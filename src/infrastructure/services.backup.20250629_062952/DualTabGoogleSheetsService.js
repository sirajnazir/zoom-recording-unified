...
class DualTabGoogleSheetsService {
    ...
    async _updateRawTab(originalRecording, source) {
        ...
        const rowData = [
            originalRecording.id || originalRecording.uuid,
            originalRecording.meeting_id || processedData.meetingId || '',
            ...
            // Use processed duration if available, otherwise calculate from original
            processedData.duration || Math.round((originalRecording.duration || 0) / 60), // minutes
            originalRecording.host_email || '',
            originalRecording.host_name || originalRecording.host_email?.split('@')[0] || '',
            // Use processed participant count if available
            processedData.participantCount || originalRecording.participant_count || 0,
            originalRecording.recording_type || 'cloud_recording',
            // Use processed file size if available
            processedData.fileSize || this._bytesToMB(originalRecording.file_size || 0),
    ...
    async _generateSmartData(processed, original, source) {
        ...
        const uuid = processedData.uuid || original.uuid || original.id || this._generateUUID();
        ...
        return {
            ...
            participantCount: enhancedMetadata.participantCount || processedData.participantCount || original.participant_count || 0,
            meetingId: original.meeting_id || original.id || '',
            
            // Recording details - use processed data
            duration: processedData.duration || Math.round((original.duration || 0) / 60), // minutes
            ...
            speakerCount: transcriptAnalysis.speakers?.length || enhancedMetadata.speakerCount || 0,
            primarySpeaker: transcriptAnalysis.primarySpeaker?.name || enhancedMetadata.primarySpeaker || '',
            ...
            driveFolder: fileManagement.driveFolder,
