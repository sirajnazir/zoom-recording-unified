        ...
        try {
            ...
            for (const file of recordingDetails.recording_files) {
                try {
                    ...
                    // NEW: Cloud Lightweight Mode - Skip specific file types
                    if (skipVideo && mappedType === 'video') {
                        this.logger.info(`☁️ CLOUD LIGHTWEIGHT: Skipping video file: ${fileName}`);
                        continue;
                    }
                    
                    if (skipAudio && mappedType === 'audio') {
                        this.logger.info(`☁️ CLOUD LIGHTWEIGHT: Skipping audio file: ${fileName}`);
                        continue;
                    }
                    
                    // Skip heavy files in lightweight mode
                    if (lightweight && heavyFileTypes.includes(mappedType)) {
                        this.logger.info(`⏭️ Skipping heavy file in lightweight mode: ${mappedType} (${fileName})`);
                        continue;
                    }
                    
                    // In lightweight mode, only download critical files
                    if (lightweight && !criticalFileTypes.includes(mappedType)) {
                        this.logger.info(`⏭️ Skipping non-critical file in lightweight mode: ${mappedType} (${fileName})`);
                        continue;
                    }
