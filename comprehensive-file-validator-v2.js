...
class ComprehensiveFileValidator {
    ...
    async initialize() {
        console.log('🚀 Initializing Comprehensive File Validator V2...\n');
        console.log('📌 Now with proper UUID format handling (hex ↔️ base64)\n');
    ...
    // Check if string is likely hex format
    isHexUuid(str) {
        // Check if it's 32 hex chars (possibly with hyphens)
        const cleanStr = str.replace(/-/g, '');
        return /^[0-9a-f]{32}$/i.test(cleanStr);
    }

    // Check if string is likely base64 format
    isBase64Uuid(str) {
        // Base64 UUIDs typically end with == and contain +/
        return /^[A-Za-z0-9+/]+=*$/.test(str) && str.length > 20;
    }

    // Normalize UUID to base64 format
    normalizeUuid(uuid) {
        if (!uuid) return null;
        
        if (this.isHexUuid(uuid)) {
            return this.hexToBase64(uuid);
        } else if (this.isBase64Uuid(uuid)) {
            return uuid;
        }
        
        return uuid; // Return as-is if format unclear
    }
    ...
    async loadFromSheets() {
        ...
        const tabsToLoad = [
            { name: 'A Recordings Multi-Tab', range: 'A:Z' },
            { name: 'B Recordings Multi-Tab', range: 'A:Z' },
            { name: 'C Recordings Multi-Tab', range: 'A:Z' },
            { name: 'Raw Master Index', range: 'A:Z' },
            { name: 'Standardized Master Index', range: 'A:Z' }
        ];
        ...
        for (const tab of tabsToLoad) {
            try {
                ...
                if (rows.length > 1) {
                    ...
                    console.log(`   Loading ${tab.name}: ${rows.length - 1} records`);

                    for (let i = 1; i < rows.length; i++) {
                        const row = rows[i];
                        if (!row || row.length === 0) continue;

                        const rawUuid = uuidIndex >= 0 ? row[uuidIndex] : null;
                        const meetingId = meetingIdIndex >= 0 ? row[meetingIdIndex] : null;
                        
                        if (rawUuid || meetingId) {
                            // Normalize UUID to base64 format
                            const normalizedUuid = this.normalizeUuid(rawUuid);
                            ...
                            if (normalizedUuid) {
                                this.sourceRecordings.fromSheets.set(normalizedUuid, recording);
                                
                                // Track UUID conversions
                                if (rawUuid !== normalizedUuid) {
                                    this.validationResults.uuidConversions.push({
                                        original: rawUuid,
                ...
                console.log(`   ⚠️  Could not load ${tab.name}: ${error.message}`);
            }
        }
        
        console.log(`   📝 Converted ${this.validationResults.uuidConversions.length} hex UUIDs to base64 format`);
    }
            ...
            for (const dir of localDirs) {
                if (dir.startsWith('M:') && dir.includes('U:')) {
                    ...
                    if (stats.isDirectory()) {
                        ...
                        const recordingInfo = {
                            meetingId,
                            uuid: uuid, // Already in base64 format
                            files: fileDetails,
                        ...
                        if (source === 'A') {
                            this.sourceRecordings.fromLocalA.set(uuid, recordingInfo);
                        } else if (source === 'B') {
                            this.sourceRecordings.fromLocalB.set(uuid, recordingInfo);
                        } else {
                            this.sourceRecordings.fromLocalC.set(uuid, recordingInfo);
                        }
                    }
                }
            }
            ...
            do {
                ...
                for (const item of response.data.files) {
                    if (item.mimeType === 'application/vnd.google-apps.folder') {
                        ...
                        if (item.name.includes('_M_') || item.name.includes('U_') || item.name.includes('U:')) {
                            ...
                            if (meetingIdMatch || uuidMatch) {
                                ...
                                // Use UUID as key (already in base64 format from folder name)
    ...
    async validateAllRecordings() {
        console.log('\n\n🔍 VALIDATING ALL RECORDINGS WITH PROPER UUID MATCHING...');
        console.log('================================================================================\n');
        
        // Create a master list of all unique recordings
        const allRecordings = new Map();
        
        // Add from sheets (already normalized to base64)
        ...
        for (const [uuid, recording] of this.sourceRecordings.fromLocalA) {
            if (allRecordings.has(uuid)) {
                allRecordings.get(uuid).sources.push('localA');
                allRecordings.get(uuid).localFiles = recording.files;
            } else {
                allRecordings.set(uuid, { ...recording, sources: ['localA'] });
            }
        }
        
        // Similar for B and C
        for (const [uuid, recording] of this.sourceRecordings.fromLocalB) {
            if (allRecordings.has(uuid)) {
                allRecordings.get(uuid).sources.push('localB');
            } else {
                allRecordings.set(uuid, { ...recording, sources: ['localB'] });
            }
        }
        
        for (const [uuid, recording] of this.sourceRecordings.fromLocalC) {
            if (allRecordings.has(uuid)) {
                allRecordings.get(uuid).sources.push('localC');
            } else {
                allRecordings.set(uuid, { ...recording, sources: ['localC'] });
            }
        }
    ...
    async validateRecording(key, recording) {
        ...
        const driveRecording = this.sourceRecordings.fromDrive.get(key) || 
                             Array.from(this.sourceRecordings.fromDrive.values()).find(r => 
                                 r.meetingId === recording.meetingId || 
                                 r.uuid === key ||
                                 (recording.rawUuid && r.uuid === recording.rawUuid)
                             );
    ...
    async standardizeFileNames() {
        ...
        // Save renaming plan
        const planPath = 'validation-reports/file-renaming-plan-v2.json';
    ...
    async validateCoachStudentShortcuts() {
        ...
        // Save shortcuts plan
        const shortcutsPath = 'validation-reports/coach-shortcuts-needed-v2.json';
    ...
    async generateReport() {
        console.log('\n\n📊 GENERATING COMPREHENSIVE REPORT V2...');
        console.log('================================================================================\n');
        ...
        const report = {
            ...
            summary: {
                ...
                missingInDrive: this.validationResults.missingFiles.filter(f => f.issue === 'NOT FOUND IN DRIVE').length,
                missingFiles: this.validationResults.missingFiles.filter(f => f.issue === 'FILES MISSING IN DRIVE').length,
                ...
                uuidConversions: this.validationResults.uuidConversions.length
            },
            criticalIssues: {
        ...
        if (report.criticalIssues.missingRecordings.length > 0) {
            ...
            report.criticalIssues.missingRecordings.forEach((rec, idx) => {
                ...
                console.log(`   UUID (Base64): ${rec.uuid || 'N/A'}`);
                console.log(`   Original UUID: ${rec.rawUuid || 'N/A'}`);
        ...
        const reportPath = `validation-reports/comprehensive-validation-v2-${timestamp}.json`;
        ...
        console.log(`UUID format conversions: ${report.summary.uuidConversions}`);
        console.log(`\n⚠️  Missing recordings: ${report.summary.missingInDrive}`);
        console.log(`⚠️  Missing files: ${report.summary.missingFiles}`);
        console.log(`✅ Successful validations: ${report.summary.successfulValidations}`);
        
        console.log(`\n📄 Detailed report saved to: ${reportPath}`);
        
        // Show UUID conversion examples
        if (this.validationResults.uuidConversions.length > 0) {
            console.log('\n\n📝 UUID FORMAT CONVERSIONS (first 5):');
            console.log('================================================================================');
            this.validationResults.uuidConversions.slice(0, 5).forEach((conv, idx) => {
                console.log(`${idx + 1}. ${conv.original} → ${conv.normalized}`);
